...
| [0064](0064-security-slice4-payment-amount-reconciliation.md) | Security Slice 4 (payments/secrets): reconcile Stripe's authoritative amount_total against what the invoice owes before marking it paid — an underpaid session still records the payment (never drop a real charge) but does not auto-satisfy the invoice, advance the funnel, or fire payment workflows, and alerts the operator; secret handling (per-tenant Stripe key, replay-guarded webhook, no secrets logged) audited and confirmed | Accepted |
| [0065](0065-security-slice5-logging-audit-final-defenses.md) | Security Slice 5 (logging/audit/final defenses): pip-audit CI gate on the pinned dependency tree, tenant-attributable auth-audit log lines + lockout alerts in hosted mode, docs/SECURITY.md playbook (rotation, session eviction, incident response), CSP + no-secrets-in-logs locked with regression tests | Accepted |
| [0066](0066-native-api-opaque-session-boundary.md) | Native API auth boundary: host-first tenant resolution, bearer-only opaque rotating sessions, exact resource guest scopes, origin-scoped Keychain storage, and a scoped `/api/v1` OpenAPI contract | Accepted |
| [0067](0067-native-client-delivery-slice.md) | Native client delivery (iOS M3): design handoff mapped onto the four guest principals (no invented client account), guest-scoped gallery/document/booking reads, gallery-guest favorite toggle with proofing cap, bearer-authenticated media routes, accept/sign/pay stay web | Accepted |
| [0068](0068-ai-capability-in-app-consolidation.md) | AI capability lives in the app's deployable unit: each capability resolves to in-process Python OR a direct hosted-API vendor call — never a separate self-hosted sidecar or homelab; re-pins the consolidation target (captions first, vision second, Dionysus reframed, Aphrodite unchanged), preserving the registry/ai_runs/shadow/flag-rollback strangler machinery | Proposed |
| [0069](0069-sidecar-credential-hygiene.md) | Sidecar credential hygiene: sets the interim rotation policy (runbook procedure over the ADR 0065 cadence; inbound gates already constant-time + fail-disarmed) and the transport rule (https on non-loopback), ships a non-auth startup cleartext-transport warning, and defers hard scheme-enforcement + any rotation mechanism as red-light — most of the surface is deleted by ADR 0068 consolidation | Proposed |
| [0070](0070-ios-app-store-distribution.md) | iOS distribution (resolves O3): one free multi-tenant companion app on the public App Store — studio chosen at sign-in via host-based `GET /api/v1/tenant` bootstrap, revenue stays on the web $20/mo Stripe subscription (no Apple IAP in v1), TestFlight → App Store with a reviewer demo studio; personal-studio-only and white-label binaries rejected | Proposed |
