...
| [0051](0051-hosted-recovery-and-ownership.md) | Hosted recovery & ownership: stateless single-use password reset, one-click full-studio export, and self-serve delete with durable exact-subscription cancellation, permanent slug retirement, and internal trash parking | Accepted |
...
| [0057](0057-hosted-backups-and-ci-hosted-suite.md) | Hosted backups: atomic sanitized generations, encrypted manifest-last off-site commit, versioned allowlisted media, positive monitoring, restore gates, and full hosted CI coverage | Accepted |
...
| [0067](0067-native-push-notification-boundary.md) | Native push boundary: owner-session device binding, encrypted APNs tokens, transactional per-device outbox, durable retry leases, contextual permission, and authorization-aware deep links | Accepted |
| [0068](0068-native-content-suggestion-boundary.md) | Native caption suggestions: default-off immutable session-bound operations, one paid-call claim, memory-only candidate review, and explicit versioned draft save | Accepted |
