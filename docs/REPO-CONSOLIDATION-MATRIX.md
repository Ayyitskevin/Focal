...
> **Branding note (July 2026):** This matrix describes the Focal consolidation plan. The repository and runtime may still use legacy `mise`/`MISE_*` identifiers; those are intentionally not mass-renamed in this branding pass.

> What to extract from each sibling repository, where it lands in Focal, what to discard,
> and how confident we are. **We do not merge Git histories or copy whole apps** — we
> rebuild/extract only the domain capability that belongs inside Focal, conforming to
> Focal's conventions and shared infrastructure.
...
- Claims about **Focal's own integration code** are **[CODE]** — verified by reading the
  files named below.
- Claims about **sibling-repo internals** are **[DOC]/[INFER]** — sourced from the
  ...
  the *Focal-side contract* each integration implements. Exact sibling file/function
...
where a specific sibling filename is INFER, because Focal's own integration code already
...
| Repo | Keep? | Target Focal module | First action | Consolidation | Conf. |
| --- | --- | --- | --- | --- | --- |
...
| **Infrastructure to discard on cutover** | Argus's own deploy/systemd, its SQLite for run state *as a second authority*, duplicated auth, any separate UI once review moves into Focal. Keep Argus as a **stateless worker** (run cache + reproducible outputs only). **[DOC]** |
...
| **Confidence** | **High** (Focal contract is [CODE]). |
...
| **Infrastructure to discard** | **Do not reproduce** Plutus signup, tenant, subscription, Stripe, or a separate storefront — Focal already owns clients, galleries, invoices, payment state, operator identity. Discard duplicated auth/deploy/queue. **[DOC]** (audit §3.2: sales engine overlaps Hestia) |
...
| **Exact source — Focal side** | `app/platekit.py` (`is_enabled`, `packs_for_client`, `notify_argus_complete` → `galleries.platekit_last_*`); `app/caption_ai.py` (Odysseus caption draft); config bridge `MISE_PLATEKIT_API_BASE`/`_TOKEN` with legacy `MISE_DIONYSUS_*` fallback; UI in `app/admin/studio.py`, `app/admin/content.py`, `app/admin/recurring.py`. **[CODE]** |
...
| **Exact source — Focal side** | **None yet** — not integrated in Focal today. Would reuse `assets.argus_hero_potential` / `argus_keeper_score` as upstream signals and the `jobs` queue. **[INFER]** |
...
| **Infrastructure to discard** | Its own media duplication, client links, separate run store as a second authority. Share Focal's analysis signals + job ledger. **[DOC]** |
...
| **Confidence** | **Medium** (no Focal code yet; capability proven elsewhere). |
...
| **Exact source — Focal side** | **None** — intentionally out of the first consolidation phase. **[INFER]** |
...
| **Infrastructure to discard** | Treat as isolated worker sharing Focal's worker/audit substrate; no authoritative client/money state. |
| **Target module** | `products` (new), **optional/later**. Only if current Focal clients/workflows show near-term value. |
...
| **Explicitly excluded from Focal** | Multi-tenancy; public SaaS signup; studio-subscription billing; tenant-scoped duplication; **a Hestia runtime dependency**; **a Hestia DB migration**. |
...
reduce repo count. Inside Focal this means every capability uses:

| Shared substrate | Focal home |
| --- | --- |
...
each engine into a **stateless, contract-true worker** Focal can drive, compare, meter, and
