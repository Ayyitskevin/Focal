...
| 2.2-backend | **Real internal vision adapter** — `InternalVisionChallengerAdapter`: Qwen3-VL (32B) on `mickeybot` via a local OpenAI-compatible endpoint (Ollama), auto-registered as the VISION challenger when `MISE_VISION_CHALLENGER_URL` is set. Sends capped web derivatives; never raises; cost 0 (local). See [ADR 0007](adr/0007-vision-challenger-qwen3-vl-local.md). | Conforms to contract; mock-tested; dormant by env (no secret in repo). | Unset the URL → challenger drops out, shadow inert. | ✅ in PR |
