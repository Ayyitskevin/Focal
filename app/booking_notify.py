...
def _link_studio(booking_id: int, inquiry_id: int | None) -> None:
    ...
    if b["reschedule_of"]:
        prev = db.one("SELECT client_id, project_id FROM bookings WHERE id=?",
                      (b["reschedule_of"],))
        if prev and prev["client_id"]:
            db.run("UPDATE bookings SET client_id=?, project_id=? WHERE id=?",
    ...
    if b["creates_notion_session"]:
        ...
        pid = db.run("""INSERT INTO projects (client_id, title, shoot_date, notes)
                        VALUES (?,?,?,?)""",
                     (cid, title, b["start_utc"][:10],
                      notion_sync.intake_summary(b) or None))
...
def confirm(booking_id: int) -> None:
    ...
    details = (f"{b['event_desc']}\n\n" if b["event_desc"] else "") + \
              f"Manage this booking: {_manage_url(b['token'])}"
    gcal_link = ics.google_link(summary=summary, details=details, location=loc,
                                start_utc=b["start_utc"], end_utc=b["end_utc"])

    if not mailer.configured():
        log.error("booking %s confirmed but mailer not configured — no emails sent",
                  booking_id)
    else:
        invite = {"filename": "invite.ics", "method": "REQUEST",
                  "content": ics.build(
                      uid=uid, summary=summary, description=details, location=loc,
                      start_utc=b["start_utc"], end_utc=b["end_utc"],
                      organizer_email=config.GMAIL_USER, attendee_email=b["email"])}
        client_body = (
            f"Hi {b['name']},\n\n"
        ...
        try:
            mailer.send(b["email"], f"Booking confirmed — {b['event_name']}",
                        client_body, reply_to=config.GMAIL_USER, ics=invite)
        ...
        try:
            ...
            mailer.send(config.GMAIL_USER,
                        f"Booking — {b['name']} · {b['event_name']} · {biz_when}",
                        kevin_body, reply_to=b["email"], ics=invite)
        except Exception as e:
    ...
    try:
        iid = db.run(
            ...
            (b["name"], b["email"], None,
             ...
             "booking", b["start_utc"][:10], b["event_name"],
...
def cancelled(booking_id: int, by_admin: bool = False) -> None:
    ...
    if mailer.configured():
        cancel_ics = {"filename": "cancel.ics", "method": "CANCEL",
                      "content": ics.build(
                          uid=ics.uid_for(booking_id), summary=summary,
                          description="This booking was cancelled.",
                          location=b["location"] or "",
                          start_utc=b["start_utc"], end_utc=b["end_utc"],
                          organizer_email=config.GMAIL_USER,
                          attendee_email=b["email"], sequence=1, cancelled=True)}
        body = (f"Hi {b['name']},\n\nYour booking has been cancelled:\n\n"
                f"  {b['event_name']}\n  {cli_when}\n\n"
                f"Book a new time any time: {config.BASE_URL}/book\n\n"
                f"— {config.SITE_NAME}\n")
        try:
            mailer.send(b["email"], f"Booking cancelled — {b['event_name']}",
                        body, reply_to=config.GMAIL_USER, ics=cancel_ics)
        except Exception as e:
