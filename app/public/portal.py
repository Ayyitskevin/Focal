...
def get_live_portal(slug: str) -> "db.sqlite3.Row":
    p = db.one("""SELECT p.*, c.name AS client_name, c.company, c.usage_rights
...
async def view(request: Request, slug: str):
    ...
    if not _has_access(request, p["id"]):
        return templates.TemplateResponse(request, "public/portal_pin.html",
                                          {"p": p, "error": None})
    ...
    db.run("UPDATE portals SET visits=visits+1, last_visit=datetime('now') WHERE id=?",
           (p["id"],))
    galleries = db.all_("""SELECT * FROM galleries WHERE client_id=? AND published=1
                           ORDER BY created_at DESC""", (p["client_id"],))
    crops = db.all_("""SELECT DISTINCT a.*, g.title AS gallery_title
                       FROM favorites f
                       JOIN assets a ON a.id=f.asset_id
                       JOIN galleries g ON g.id=a.gallery_id
                       WHERE g.client_id=? AND g.published=1
                         AND a.kind='photo' AND a.status='ready'
                       ORDER BY g.created_at DESC, a.id""", (p["client_id"],))
    ...
    fav_summary = db.one("""SELECT COUNT(DISTINCT f.asset_id) AS n_faves,
    ...
    return templates.TemplateResponse(request, "public/portal.html",
                                      {"p": p, "galleries": galleries, "crops": crops,
                                       "brand": brand,
                                       "ratios": [ps["slug"] for ps in presets.active()],
                                       "prev_visit": prev_visit, "changes": changes,
...
async def check_pin(request: Request, slug: str, pin: str = Form(...)):
    ...
    if security.pin_locked(ip, -p["id"]):
        return templates.TemplateResponse(
            request, "public/portal_pin.html",
    ...
    resp.set_cookie(_cookie_name(p["id"]), security.sign(f"portal:{p['id']}"),
...
def _client_asset(portal: "db.sqlite3.Row", asset_id: int) -> "db.sqlite3.Row":
    a = db.one("""SELECT a.* FROM assets a JOIN galleries g ON g.id=a.gallery_id
...
async def thumb(request: Request, slug: str, asset_id: int):
    ...
    path = (config.MEDIA_DIR / str(a["gallery_id"]) / "thumb"
...
async def crops_zip(request: Request, slug: str):
    p = get_live_portal(slug)
    _require_access(request, p["id"])
    rows = db.all_("""SELECT DISTINCT a.* FROM favorites f
                      JOIN assets a ON a.id=f.asset_id
                      JOIN galleries g ON g.id=a.gallery_id
                      WHERE g.client_id=? AND g.published=1
                        AND a.kind='photo' AND a.status='ready'
                      ORDER BY a.id""", (p["client_id"],))
    ratios = [ps["slug"] for ps in presets.active()]
    files = []
    for a in rows:
        for ratio in ratios:
    ...
    key = hashlib.sha256(
        "|".join(f"{a['id']}:{r}" for a, r, _ in files).encode()).hexdigest()[:8]
...
async def brand_file(request: Request, slug: str, ba_id: int):
    p = get_live_portal(slug)
    _require_access(request, p["id"])
    b = db.one("SELECT * FROM brand_assets WHERE id=? AND client_id=?",
               (ba_id, p["client_id"]))
    if not b:
        raise HTTPException(status_code=404)
