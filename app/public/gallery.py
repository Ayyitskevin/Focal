...
async def view(request: Request, slug: str):
    ...
    assets = db.all_("""SELECT * FROM assets WHERE gallery_id=? AND status='ready'
                        ORDER BY section_id, position, id""", (g["id"],))
    favs = {r["asset_id"] for r in db.all_(
        "SELECT asset_id FROM favorites WHERE visitor_id=?", (visitor["id"],))}
    # this visitor's selection count per proofing section, for the progress label
    section_picks = {s["id"]: sum(1 for a in assets
                                  if a["section_id"] == s["id"] and a["id"] in favs)
                     for s in sections}
    ...
    return templates.TemplateResponse(request, "public/gallery.html",
...
async def check_pin(request: Request, slug: str, pin: str = Form(...)):
    ...
    resp.set_cookie(security.visitor_cookie_name(g["id"]), cookie_val,
                    max_age=config.SESSION_MAX_AGE, httponly=True,
                    secure=config.COOKIE_SECURE, samesite="lax", path="/")
    return resp
...
def _progress_oob(g_id: int, section_id: int | None, visitor_id: int) -> str:
    ...
    if not s or not s["proof_target"]:
        return ""
    picks = db.one("""SELECT COUNT(*) AS n FROM favorites f JOIN assets a ON a.id=f.asset_id
                      WHERE f.visitor_id=? AND a.section_id=?""",
                   (visitor_id, section_id))["n"]
    cls = "ok" if picks >= s["proof_target"] else "muted"
    return (f'<span id="proof-{section_id}" class="proof-progress {cls}" '
...
async def toggle_fav(request: Request, slug: str, asset_id: int):
    ...
    a = db.one("""SELECT a.id, a.kind, a.status, a.section_id, s.proof_target
                  FROM assets a LEFT JOIN sections s ON s.id=a.section_id
                  WHERE a.id=? AND a.gallery_id=?""", (asset_id, g["id"]))
    if not a:
        raise HTTPException(status_code=404)
    existing = db.one("SELECT 1 AS x FROM favorites WHERE visitor_id=? AND asset_id=?",
                      (visitor["id"], asset_id))
    if existing:
        db.run("DELETE FROM favorites WHERE visitor_id=? AND asset_id=?",
               (visitor["id"], asset_id))
    ...
    if a["proof_target"]:
        picks = db.one("""SELECT COUNT(*) AS n FROM favorites f
                          JOIN assets x ON x.id=f.asset_id
                          WHERE f.visitor_id=? AND x.section_id=?""",
                       (visitor["id"], a["section_id"]))["n"]
        if picks >= a["proof_target"]:
            return Response('<span class="fav-btn">&#9825;</span>', status_code=409,
                            headers={"HX-Trigger":
                                     f'{{"proof-cap":{{"target":{a["proof_target"]}}}}}'})
...
def video_comment_thread(asset_id: int) -> list[dict]:
    ...
    rows = db.all_("""SELECT id, parent_id, timecode, body, author_role, status, created_at
                      FROM video_comments
                      WHERE asset_id=? AND deleted_at IS NULL
                      ORDER BY timecode, created_at, id""", (asset_id,))
...
def resolve_comment_parent(asset_id: int, parent_raw) -> tuple[int | None, float]:
    ...
    p = db.one("SELECT timecode FROM video_comments WHERE id=? AND asset_id=? "
               "AND deleted_at IS NULL", (pid, asset_id))
    if not p:
...
def _maybe_reopen_on_reply(con, reply_id: int):
    ...
    audit.log(con, "video_comment", root["id"], "open",
              diff={"asset_id": root["asset_id"], "from": "resolved", "to": "open",
                    "cause_reply_id": reply_id}, actor="system")
    return {"root_id": root["id"], "asset_id": root["asset_id"],
...
def _live_video_asset(request: Request, slug: str, asset_id: int):
    ...
    a = db.one("SELECT id FROM assets WHERE id=? AND gallery_id=? AND kind='video' "
               "AND status='ready'", (asset_id, g["id"]))
    if not a:
        raise HTTPException(status_code=404)
    return g, visitor
...
async def add_comment(request: Request, slug: str, asset_id: int,
                      body: str = Form(...), timecode: float = Form(0.0),
                      parent_id: str = Form("")):
