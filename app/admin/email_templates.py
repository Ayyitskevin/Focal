...
MERGE_FIELDS = ["first_name", "client_name", "company", "project_title",
                "doc_title", "doc_url", "site_name"]


@router.get("/email-templates", response_class=HTMLResponse)
async def list_templates(request: Request):
    rows = db.all_("SELECT * FROM email_templates WHERE deleted_at IS NULL "
                   "ORDER BY name")
    return templates.TemplateResponse(request, "admin/email_templates.html",
                                      {"rows": rows, "merge_fields": MERGE_FIELDS,
                                       "base_url": config.BASE_URL})


@router.post("/email-templates")
async def create_template(name: str = Form(...), subject: str = Form(...),
                          body: str = Form(...)):
    ...
    tid = db.run("INSERT INTO email_templates (name, subject, body) VALUES (?,?,?)",
                 (name, subject, body))
...
async def update_template(template_id: int, name: str = Form(...),
                          subject: str = Form(...), body: str = Form(...)):
    d = db.one("SELECT id FROM email_templates WHERE id=? AND deleted_at IS NULL",
               (template_id,))
    if not d:
    ...
    if not (name and subject and body.strip()):
        raise HTTPException(status_code=400, detail="name, subject and body required")
    db.run("UPDATE email_templates SET name=?, subject=?, body=? WHERE id=?",
...
async def delete_template(template_id: int):
    db.run("UPDATE email_templates SET deleted_at=datetime('now') WHERE id=?",
           (template_id,))
