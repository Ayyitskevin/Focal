-- Relabel demo showcase from "Mise Demo" → anonymous client attribution on the public site.
UPDATE galleries
...
Turnaround: Same-week gallery'
        ELSE cs_credits END,
    cs_location=COALESCE(NULLIF(cs_location, ''), 'Western North Carolina')
WHERE id=1 AND (client_name IS NULL OR client_name IN ('', 'Mise Demo', 'Cúrate'));
